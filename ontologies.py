...
with ontology:
    ...
    class Shape(owlready2.Thing):
        pass

    class Taste(owlready2.Thing):
        pass

    class Nutrition(owlready2.Thing):
        pass

    class Country(owlready2.Thing):
        pass

    class Recipe(owlready2.Thing):
        pass

    class Category(owlready2.Thing):
        pass

    class Edible(owlready2.Thing):
        pass

    class Dietary_method(owlready2.Thing):
        pass

    class Function(owlready2.Thing):
        pass

    class Ground(owlready2.Thing):
        pass

    class Seed(owlready2.Thing):
        pass

    # Role - Relationship
    class has_color(owlready2.ObjectProperty):
        domain = [Food]
        range = [Color]

    class has_shape(owlready2.ObjectProperty):
        domain = [Tool]
        range = [Shape]

    class has_taste(owlready2.ObjectProperty):
        domain = [Food]
        range = [Taste]

    class has_nutrition(owlready2.ObjectProperty):
        domain = [Food]
        range = [Nutrition]

    class original_from(owlready2.ObjectProperty):
        domain = [Food]
        range = [Country]

    class is_ingredient_of(owlready2.ObjectProperty):
        domain = [Food]
        range = [Recipe]

    class has_ingredient(owlready2.ObjectProperty):
        domain = [Recipe]
        range = [Food]

    class is_category_of(owlready2.ObjectProperty):
        domain = [Food]
        range = [Category]

    class has_edible_part(owlready2.ObjectProperty):
        domain = [Food]
        range = [Edible]

    class has_child_food(owlready2.ObjectProperty):
        domain = [Food]
        range = [Food]

    class is_cousin_to(owlready2.ObjectProperty):
        domain = [Food]
        range = [Food]

    class has_seed_inside(owlready2.ObjectProperty):
        domain = [Food]
        range = [Seed]

    class has_dietary_method(owlready2.ObjectProperty):
        domain = [Food]
        range = [Dietary_method]

    class has_function(owlready2.ObjectProperty):
        domain = [Tool]
        range = [Function]

    class has_sub_function(owlready2.ObjectProperty):
        domain = [Tool]
        range = [Tool]

    class is_growing(owlready2.ObjectProperty):
        domain = [Food]
        range = [Ground]
    ...
    carotenoids = Nutrition(
        "carotenoids"
    )  # 类胡萝卜素 In many vegetables but high in carrots, pumpkin and green leafy vegetables
    lycopene = Nutrition("lycopene")  # 番茄红素 Tomatoes, watermelon
    lutein = Nutrition("lutein")  # 叶黄素 spinach, silverbeet, lettuce; sweet corn
    glucosinolates = Nutrition(
        "glucosinolates"
    )  # 芥子油苷-可以通过促进对致癌物进行解毒的酶来降低患某些类型癌症的风险。broccoli, cauliflower, cabbage,
    flavonoids = Nutrition(
        "flavonoids"
    )  # 类黄酮 beans, onions, leafy vegetables, tomatoes
    phenolic_acids = Nutrition(
        "phenolic_acids"
    )  # 酚酸 In most vegetables but especially potatoes
    anthocyanins = Nutrition(
        "anthocyanins"
    )  # 花青素 Red, blue/purple vegetables – eggplant
    allium_sulphur_compounds = Nutrition(
        "allium_sulphur_compounds"
    )  # 葱硫化合物 Garlic, leeks, onions, chives
    ...
    carrot = Vegetable(
        "carrot",
        has_shape=[long, conical, cylindrical],
        ...
        has_edible_part=[roots],
        is_category_of=[root_vegetables],
        is_ingredient_of=[
            vegetable_juice,
            salad,
            stir_fry,
    ...
    eggplant = Vegetable(
        ...
        has_color=[purple],
        has_nutrition=[anthocyanins, iron, fiber, vitamin_B],
        has_edible_part=[fruits],
        has_seed_inside=[seed],
        is_category_of=[fruit_vegetables],
        original_from=[asia],
        is_growing=[above_ground],
        is_ingredient_of=[stir_fry, grilled_vegetables],
    )
    cucumber = Vegetable(
        "cucumber",
        has_shape=[long, cylindrical],
        has_color=[green],
        has_nutrition=[potassium, fiber, vitamin_C],
        has_taste=[juicy, crisp],
        has_edible_part=[fruits],
        has_seed_inside=[seed],
        is_category_of=[fruit_vegetables],
        is_ingredient_of=[vegetable_juice, salad],
        has_dietary_method=[raw_and_cooked],
        is_growing=[above_ground],
    )
    ...
    potato = Vegetable(
        ...
        has_taste=[sweet],
        has_edible_part=[tubers],
        is_category_of=[tuber_vegetables],
    ...
    pumpkin = Vegetable(
        ...
        has_shape=[round],
        has_color=[yellow, orange],
        has_nutrition=[carotenoids, vitamin_A, fiber],
        has_taste=[sweet],
        original_from=[mexico, america],
        has_edible_part=[fruits],
        has_seed_inside=[seed],
        is_category_of=[fruit_vegetables],
        has_dietary_method=[raw_and_cooked],
    ...
    mushroom = Vegetable(
        ...
        has_shape=[round],
        has_color=[gray],
        has_nutrition=[iron, vitamin_D, vitamin_B, potassium, copper, phosphorus],
        has_edible_part=[fungi],
        is_category_of=[fungi_vegetables],
        original_from=[france, europe],
        has_dietary_method=[cooked],
        is_growing=[above_ground],
        is_ingredient_of=[stir_fry, grilled_vegetables],
    )
    ...
    onion = Vegetable(
        "onion",
        has_shape=[round],
        has_color=[white, purple, brown],
        has_nutrition=[allium_sulphur_compounds, flavonoids, vitamin_C],
        has_taste=[pungent],
        has_edible_part=[bulbs],
    ...
    garlic = Vegetable(
        ...
        has_shape=[round],
        has_color=[white],
        has_nutrition=[allium_sulphur_compounds, manganese, vitamin_B],
        has_taste=[pungent],
        has_edible_part=[bulbs],
        has_seed_inside=[seed],
        is_category_of=[bulb_vegetables],
        is_ingredient_of=[stir_fry, pungent_meal],
        has_dietary_method=[raw_and_cooked],
        is_growing=[under_ground],
    )
    leek = Vegetable(
    ...
    tomato = Fruit(
        ...
        has_shape=[round],
        has_color=[red],
        has_taste=[juicy, sweet],
        has_nutrition=[lycopene, flavonoids, vitamin_C],
        has_edible_part=[fruits],
        has_seed_inside=[seed],
        is_category_of=[fruit_vegetables],
        has_dietary_method=[raw_and_cooked],
        is_growing=[above_ground],
        is_ingredient_of=[vegetable_juice, salad, grilled_vegetables, vegetable_curry],
    )
    ...
    apple = Fruit(
        "apple",
        has_color=[green, red],
        has_taste=[sweet, fruity, tart],
        is_ingredient_of=[cake, fruit_juice, fruit_salad, fruit_smoothie, smoothie],
        has_shape=[round],
        has_nutrition=[sugar, fiber, vitamin_C, potassium, carbs, water],
        has_seed_inside=[seed],
    )
    banana = Fruit(
        "banana",
        ...
        is_ingredient_of=[cake, fruit_salad, fruit_smoothie, smoothie],
        has_nutrition=[
            vitamin_B,
            fiber,
            potassium,
    ...
    orange = Fruit(
        "orange",
        has_color=[orange],
        has_taste=[sweet, sour, juicy, fruity],
        is_ingredient_of=[fruit_juice, fruit_salad, fruit_smoothie, smoothie],
        has_shape=[round],
        has_nutrition=[sugar, fiber, vitamin_C, potassium, carbs, water, calcium],
        has_seed_inside=[seed],
    )
    lemon = Fruit(
        "lemon",
        ...
        has_taste=[sweet, sour, juicy, fruity],
        original_from=[china],
        is_ingredient_of=[juice],
        has_shape=[round],
        has_nutrition=[sugar, fiber, vitamin_C, potassium, carbs, water, vitamin_B],
        has_seed_inside=[seed],
    )
    plum = Fruit(
        "plum",
    ...
    avocado = Fruit(
        ...
        has_shape=[pear_shaped],
        has_color=[green],
        has_nutrition=[protein, fat, vitamin_A],
        original_from=[mexico, america],
        has_edible_part=[fruits],
        has_seed_inside=[seed],
        is_category_of=[fruit_vegetables],
    ...
    milk = Dairy(
        "milk",
        has_color=[white],
        has_nutrition=[protein, fat, vitamin_A, vitamin_B, phosphorus],
        has_taste=[sweet],
        is_ingredient_of=[smoothie, fruit_smoothie],
    )
    ...
    vegetable_juice = Recipe(
        "vegetable_juice", has_ingredient=[carrot, tomato, cucumber]
    )
    fruit_juice = Recipe(
        "fruit_juice",
    ...
    fruit_smoothie = Recipe(
        "fruit_smoothie",
        has_ingredient=[milk, yoghurt, banana, apple, orange, watermelon, lemon, lime],
    )
